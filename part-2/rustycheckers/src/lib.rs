...
pub extern "C" fn get_piece(x: i32, y: i32) -> i32 {
    ...
    let piece = engine.as_ref().get_piece(Coordinate(x as usize, y as usize));
    match piece {
        Ok(Some(p)) => p.into(),
        Ok(None) | Err(_) => -1,
    }
...
pub extern "C" fn get_current_turn() -> i32 {
    ...
    GamePiece::new(engine.as_ref().current_turn()).into()
}

// Q13: Is there a more ergonomic way to work with bitflags that's compatible with WASM?
