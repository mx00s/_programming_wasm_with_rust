;; Q1: What besides `(memory ...)` and `(func ...)` can go in `module`?
(module
  ...
  ;; Q2: What happens when `memory` is omitted or `0` pages is passed instead?
  (memory $mem 1)
  (global $currentTurn (mut i32) (i32.const 0))

  ;; Q5: Can literal numbers be written in binary, octal, or hex instead?
  ...
  ;; Q4: Do wat tools support using Lisp-style quasiquoting?
  ...
  (func $toggleTurnOwner
    (if (i32.eq (call $getTurnOwner) (get_global $WHITE))
      ...
      (else (call $setTurnOwner (get_global $WHITE)))))
  ...
  ;; We crown black pieces in row 0, white piecs in row 7
  ...
  (func $isValidMove (param $fromX i32) (param $fromY i32) (param $toX i32) (param $toY i32) (result i32)
    ...
    (set_local $target (call $getPiece (get_local $toX) (get_local $toX)))
  ...
  (func $move (param $fromX i32) (param $fromY i32) (param $toX i32) (param $toY i32) (result i32)
    (if (result i32)
      ...
      (then (call $do_move (get_local $fromX) (get_local $fromY) (get_local $toX) (get_local $toY)))
      (else (i32.const 0))))
